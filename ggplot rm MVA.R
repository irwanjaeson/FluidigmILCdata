...
p1a <- ggplot(data = scores.df, aes(x = scores.df$PC1, y = scores.df$PC2, colour = VRint, shape = VRint)) + 
  ...
  theme(legend.background = element_rect(colour = "white", fill= "white", size = 0.1), legend.key = element_rect(colour = "white", fill = "white"), legend.text = element_text(face = "bold", size = 16), legend.title = element_text(size = 16))
...
tiff('ILC2 group ggplot Route.tiff', units="in", width=17, height=6.5, res=600)

multiplot(p1a, p2a, cols = 2)
