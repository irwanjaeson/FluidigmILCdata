...
p1 <- ggplot(data = scores.df, aes(x = scores.df$PC1, y = scores.df$PC2, colour = Vaccine, shape = Vaccine)) + 
  geom_point(data = scores.df, aes(x = scores.df$PC1, y = scores.df$PC2, colour = Vaccine, shape = Vaccine), colour = "black", size = 4) +
  geom_point(data = scores.df, aes(x = scores.df$PC1, y = scores.df$PC2, colour = Vaccine, shape = Vaccine), size = 3) +
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_vline(xintercept = 30, size= 0.5) +
  geom_hline(yintercept = -2, size = 0.5) +
  ...
  geom_point(data = mean_scores, aes(x = mean_scores$PC1, y = mean_scores$PC2, colour = Vaccine), colour = "black", size = 9) +
  geom_point(data = mean_scores, aes(x = mean_scores$PC1, y = mean_scores$PC2, colour = Vaccine), size = 8) +
  theme(legend.position = "right") +
  theme(legend.key.size = unit(0.3, "cm")) + 
  theme(legend.background = element_rect(colour = "black", fill= "white", size = 0.5), legend.key = element_rect(colour = "white", fill = "white"), legend.text = element_text(size = 16), legend.title = element_text(size = 16))

p2 <- ggplot(data = scores.df, aes(x = scores.df$PC1, y = scores.df$PC4, colour = Vaccine, shape = Vaccine)) + 
...
tiff('ILC2 grouped ggplot im only.tiff', units="in", width=17, height=6.5, res=600)
